...
class Codnitive_Pardakht_Model_System_Config_Backend_Encryptedserial extends Mage_Core_Model_Config_Data
{
	...
	protected function _beforeSave()
	{
		...
		if (empty($value)) {
			Mage::throwException(Mage::helper('pardakht')->__('%s is required.',
					Mage::helper('pardakht')->__($fieldConf->label)));
		}
		...
		if (!$validator->isValid($value)) {
			Mage::throwException(Mage::helper('pardakht')->__('%s is not valid.',
					Mage::helper('pardakht')->__($fieldConf->label)));
		}
		...
		try {
			...
			try {
				...
				curl_setopt($ch, CURLOPT_URL, Mage::helper('pardakht')->getConUrl().'update/');
