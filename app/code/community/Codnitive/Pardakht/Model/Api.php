...
class Codnitive_Pardakht_Model_Api
{
	...
	protected function _connect()
	{
		try {
			...
			if (!$client) {
				Mage::log($client, null, 'codnitive_pardakht.log', true);
...
//			exit;
			...
			Mage::log($e->getMessage(), null, 'codnitive_pardakht.log', true);
			return false;
		}
		catch (Exception $e){
			Mage::log($e->getMessage(), null, 'codnitive_pardakht.log', true);
	...
	public function sendOrderToPardakht($data)
	{
		$helper     = Mage::helper('pardakht');
		...
		$region     = (string) $address->getPardakhtRegionId();
		$city       = (string) $address->getPardakhtCity();
		...
		$bank       = (string) '';
		$marketer   = (string) 'fs';
		$url        = (string) Mage::getModel('pardakht/method_online')->getResponseUrl();
		$username   = (string) $this->_getUsername();
		$password   = (string) $this->_getPassword();
...
//		Mage::log($password);
		...
		/*$status = $client->FSetOrder(
			$firstName, $lastName, $gender, $email, $region, $city, $street[0],
			$postcode, $telephone, $shippingId, $products, $paymentId, $draft,
			$bank, $marketer, $url, 'rama227', 'raha227227'
		);*/
		$result = urldecode($result);
//		$this->_getCheckout()->setPardakhtResponseResult($result);
	...
	{
		$helper     = Mage::helper('pardakht');
		$data       = Mage::getModel('pardakht/setorder')->loadByOrderId($orderId);

		Mage::log($data);

		$resNum     = (string) $data->getPardakhtOrderNumber();
