...
(function($) {
	Codnitive =
	{
		pardakht: {
			...
			{
//				var pardakhtRegionCity = '.pardakht-shipping-region-city';
//				var mageRegionCity   = '.mage-shipping-region-city';
//				if ($this.id === 'billing:country_id') {
//					pardakhtRegionCity = '.pardakht-billing-region-city';
//					mageRegionCity   = '.mage-billing-region-city';
//				}
				var pardakhtRegionCity = '.pardakht-' + type + '-region-city';
				var mageRegionCity   = '.mage-' + type + '-region-city';

				$(mageRegionCity).removeClass('hidden');
				$(pardakhtRegionCity).addClass('hidden');
				if ($this.value === 'IR') {
					$(pardakhtRegionCity).removeClass('hidden');
					$(mageRegionCity).addClass('hidden');
				}
			},

//			checkAddressForm: function(type)
//			{
//				var countryFieldId = type + ':country_id';
//				var regionFieldId  = 'id_ostan';
//				var cityFieldId    = 'shahr';
//
//				if (document.getElementById(countryFieldId).value === 'IR') {
//					var regionValue = $('#' + regionFieldId + ' option:selected').text();
////					var regionText  = $('#' + regionFieldId + ' option[value=\'' + regionValue + '\']');
//					alert(regionValue);
//				}
//			},
			...
			{
				var pardakhtSelector = '#' + type + '_pardakht_region_id';
				var mageSelector   = '#' + type + ':region_id';
				if (type !== 'billing' && type !== 'shipping') {
					mageSelector = 'region_id';
				}
				var regionText     = $(pardakhtSelector + ' option:selected').text();
			...
			{
				var pardakhtSelector = '#' + type + '_pardakht_city';
//                var mageSelector   = '#' + type + ':city';
			...
			{
				alert(type);
				ldMenu(document.getElementById(type + '_pardakht_region_id').selectedIndex, type + '_pardakht_city');
			}

			/*billingSave: function()
			{
				var regionSelector = '#billing_pardakht_region_id';
				var regionId  = $(regionSelector).val();
				var condition = regionId == '' || regionId == null || regionId == 0;
				if (condition) {
					var mageSelector   = '#billing:region_id';
					if ($(mageSelector).val()) {
						var regionText = $(mageSelector + ' option:selected').text();
					}
					else {
						var regionText = $('#billing:region').val();
					}
					alert($('#billing:region').val());
					$(regionSelector + ' option[text="' + regionText + '"]').prop('selected', true);
				}
				return false;
				billing.save()
			}*/
...
//			alert(1);
//			Codnitive.pardakht.flipFields(this);
//		});
//	});
})(jQuery);
...
function loadCities(type, cityCode)
{
	ldMenu(document.getElementById(type + '_pardakht_region_id').selectedIndex, type + '_pardakht_city');
	(function($) {
		$('#' + type + '_pardakht_city option[value="' + cityCode + '"]').prop('selected', true);
	})(jQuery);
}
